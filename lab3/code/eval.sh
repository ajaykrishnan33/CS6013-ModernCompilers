...
function compile_and_run_taco_java {
	cd $DIR/results/inputs/programs/$1
	echo $1
	for prog in $(ls *.java)
	do
		echo $prog
...
# expects folder name as parameter
function translate_to_taco2_java {
	curr="$DIR/results/inputs/programs/$1"
	cd $curr
	echo $1
	...
	do
		echo $prog
		cd $DIR/TACoJavaParser
		sudo java P3 < $curr/$prog > $new_folder/$prog
	done
	echo -e "\n\n"
}

# expects folder name as parameter
function parse_taco2_java {
	cd $DIR/results/outputs/programs/$1
	echo $1
	for prog in $(ls *.java)
	do
		echo $prog
		cd $DIR/TACoJava2Parser
...
# expects folder name as parameter
function compile_and_run_taco2_java {
	cd $DIR/results/outputs/programs/$1
	echo $1
	for prog in $(ls *.java)
	do
...
then
	...
	compile_and_run_taco_java "CONT17"	
	echo -e "\n\nRunning TACoJavaParser on TACoJava programs\nResults in results/outputs/programs\n\n"
	translate_to_taco2_java "CONT17"
	echo -e "\n\nRunning TACoJava2Parser on the generated programs\n\n"
	cd $DIR/TACoJava2Parser
	javac Main.java
	parse_taco2_java "CONT17"
	echo -e "\n\nCompiling and running TACoJava2 programs\nResults in results/outputs/values\n\n"
	compile_and_run_taco2_java "CONT17"
else
	echo -e "\n\nCompiling and running TACoJava programs.\nResults in results/inputs/values\n\n"
	...
	for folder in $(ls)
	do
		translate_to_taco2_java "$folder"
	done
	...
	for folder in $(ls)
	do
		compile_and_run_taco2_java "$folder"
	done
fi
